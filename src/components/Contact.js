...
const Contact = () => {
    ...
    const showContact = async () => {
        const datas = await fetch(`${BASE_URL}/showContacts`)
    ...
    const handleChangeStatus = async (newValue, id) => {
        const result = await fetch(`${BASE_URL}/contactsStatus/${id}`, {
            method: "Put",
    ...
    const deleteData = async (_id) => {
        const datas = await fetch(`${BASE_URL}/deleteContacts/${_id}`, {
            method: "delete"
        })
        const results = await datas.json()
        if (results) {
    ...
    const filteredData = showContacts.filter(item =>
    ...
    const downloadCSV = (array) => {
        ...
        link.setAttribute('download', 'contactus.csv');
        link.click();
