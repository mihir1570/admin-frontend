...
const Getintouch = () => {
    ...
    const [showGetintouchs, setShowGetintouchs] = useState([]);
    ...
    const showGetintouch = async () => {
        const datas = await fetch(`${BASE_URL}/showGetintouchs`);
        const results = await datas.json();
        if (results) {
            setShowGetintouchs(results);
            console.log(results);
        }
    };
    ...
    const handleChangeStatus = async (newValue, id) => {
        const result = await fetch(`${BASE_URL}/getintouchStatus/${id}`, {
            method: "Put",
    ...
    // Delete data Function's
    const deleteData = async (_id) => {
        const datas = await fetch(`${BASE_URL}/deleteGetintouchs/${_id}`, {
            method: "delete"
        });
        const results = await datas.json();
        if (results) {
    ...
    const filteredData = showGetintouchs.filter(item =>
    ...
    const downloadCSV = (array) => {
        ...
        link.setAttribute('download', 'getintouch.csv');
        link.click();
    ...
    const columns = [
...

        {
            name: 'Message',
            cell: row => {
                const truncatedMessage = row.message.length > 50 ? `${row.message.slice(0, 50)}...` : row.message;
                return (
